...
CData's Model Context Protocol (MCP) Server for Amazon DynamoDB

:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Amazon DynamoDB (beta)](https://www.cdata.com/download/download.aspx?sku=DDZK-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Amazon DynamoDB supported by the [CData JDBC Driver for Amazon DynamoDB](https://www.cdata.com/drivers/dynamodb/jdbc).

CData JDBC Driver connects to Amazon DynamoDB by exposing them as relational SQL models.

This server wraps that driver and makes Amazon DynamoDB data available through a simple MCP interface, so LLMs can retrieve live information by asking natural language questions — no SQL required.

## Setup Guide
1. Clone the repository:
      ```bash
      git clone https://github.com/cdatasoftware/amazon-dynamodb-mcp-server-by-cdata.git
      cd amazon-dynamodb-mcp-server-by-cdata
      ```
2. Build the server:
      ```bash
      mvn clean install
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/dynamodb/download/jdbc](https://www.cdata.com/drivers/dynamodb/download/jdbc)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for Amazon DynamoDB\`
        * (Mac/Linux) `/Applications/CData JDBC Driver for Amazon DynamoDB/`
    * Run the command `java -jar cdata.jdbc.amazondynamodb.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.amazondynamodb.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `amazon-dynamodb.prp`) using the following properties and format:
    * **Prefix** - a prefix to be used for the tools exposed
    * **ServerName** - a name for your server
    * **ServerVersion** - a version for your server
    * **DriverPath** - the full path to the JAR file for your JDBC driver
    * **DriverClass** - the name of the JDBC Driver Class (e.g. cdata.jdbc.amazondynamodb.AmazonDynamoDBDriver)
    * **JdbcUrl** - the JDBC connection string to use with the CData JDBC Driver to connect to your data (copied from above)
    * **Tables** - leave blank to access all data, otherwise you can explicitly declare the tables you wish to create access for
      ```env
      Prefix=amazondynamodb
      ServerName=CDataAmazonDynamoDB
      ServerVersion=1.0
      DriverPath=PATH\TO\cdata.jdbc.amazondynamodb.jar
      DriverClass=cdata.jdbc.amazondynamodb.AmazonDynamoDBDriver
      JdbcUrl=jdbc:amazondynamodb:InitiateOAuth=GETANDREFRESH;
      Tables=
      ```
...
```json
{
    ...
    "method": "tools/call",
    "params": {
        "name": "amazon_dynamodb_get_tables",
        "arguments": {}
    }
}
```

#### amazon_dynamodb_get_columns
```json
{
    "jsonrpc": "2.0",
    "id": 2,
    "method": "tools/call",
    "params": {
        "name": "amazon_dynamodb_get_columns",
        "arguments": {
            "table":  "Account"
        }
    }
...
```json
{
    ...
    "method": "tools/call",
    "params": {
        "name": "amazon_dynamodb_run_query",
        "arguments": {
